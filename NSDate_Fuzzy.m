...
- (NSString *)distanceOfTimeInWordsFromNow
{
	...
	if (t < 50 * MINUTE) {
		return [NSString stringWithFormat:@"%d minutes ago", round(t / MINUTE)];
	}
	if (t < 90 * MINUTE) {
		return @"about an hour ago";
	}
	if (t < 18 * HOUR) {
		return [NSString stringWithFormat:@"%d hours ago", round(t / HOUR)];
	}
	if (t < 2 * DAY) {
		return @"about a day";
	}
	if (t < 20 * DAY) {
		return [NSString stringWithFormat:@"%d days ago", round(t / DAY)];
	}
	if (t < 1 * MONTH) {
